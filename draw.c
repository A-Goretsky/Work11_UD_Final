...
void scanline_convert( struct matrix *points, int i, screen s, zbuffer zb, color c ) {
	...
	tx = points->m[0][top]; mx = points->m[0][mid]; bx = points->m[0][bot];
	ty = points->m[1][top]; my = points->m[1][mid]; by = points->m[1][bot];
	tz = points->m[2][top]; mz = points->m[2][mid]; bz = points->m[2][bot];

	dy = my - by;

	c.red = ((tx * by) + 128) % 256 ;
	c.green = ((mx * my) + 217) % 256;
	c.blue = ((bx * ty) + 228) % 256;
	...
	if (ty - by != 0) {
		if (dy != 0) {
			...
			for (ctr = 0; ctr < dy; ctr++) {
				drawscanline(bx + ctr * xmb, by + ctr, bz + ctr * zmb, bx + ctr * xtb, by + ctr, bz + ctr * ztb, s, zb, c);
			}
		}
		...
		if (dy != 0) {
			...
			for (ctr = 0; ctr < dy; ctr++) {
				y = my + ctr;
				drawscanline(mx + ctr * xtm, y, mz + ctr * ztm, bx + (my-by) * xtb + ctr * xtb, y, bz + my - by + ctr * ztb, s, zb, c);
			}
		}
	}
}
