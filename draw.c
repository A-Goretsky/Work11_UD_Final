...
void scanline_convert(struct matrix* points, int i, screen s, zbuffer zb, color c) {
	...
	int mid;
	int bot;
	int tx, ty, tz, mx, my, mz, bx, by, bz, dy;
	...
	if (ty - by != 0) {
		if (dy != 0) {
			xmb = ((double) (mx - bx)) / (my - by);
			xtb = ((double) (tx - bx)) / (ty - by);
			zmb = ((double) (mz - bz)) / (my - by);
			ztb = ((double) (tz - bz)) / (ty - by);
			for (ctr = 0; ctr < dy; ctr++) {
				drawscanline(bx + ctr * xmb, by + ctr, bz + ctr * zmb, bx + ctr * xtb, by + ctr, bz + ctr * ztb, s, zb,
				             c);
			}
		}
		...
		if (dy != 0) {
			xtb = ((double) (tx - bx)) / (ty - by);
			xtm = ((double) (tx - mx)) / (ty - my);
			ztb = ((double) (tx - bz)) / (ty - by);
			ztm = ((double) (tz - mz)) / (ty - my);

			int y;
			for (ctr = 0; ctr < dy; ctr++) {
				y = my + ctr;
				drawscanline(mx + ctr * xtm, y, mz + ctr * ztm, bx + (my - by) * xtb + ctr * xtb, y,
				             bz + my - by + ctr * ztb, s, zb, c);
			}
		}
	}
}

void drawscanline(int x0, int y0, double z0, int x1, int y1, double z1, screen s, zbuffer zb, color c) {
	int ctr;
	int xt, zt, dz;
	if (x1 < x0) {
		xt = x0;
	...
	for (ctr = 0; ctr < (x1 - x0); ctr++) {
		plot(s, zb, c, x0 + ctr, y0, z0 + ctr * dz);
	}
}

int topg(struct matrix* polygons, int i) {
	int ans = i;
	int ay, by, cy;
	ay = polygons->m[1][i];
	by = polygons->m[1][i + 1];
...
int midg(struct matrix* polygons, int i) {
	return 3 * (i + 1) - (botg(polygons, i) + topg(polygons, i));
}

int botg(struct matrix* polygons, int i) {
	int ans = i;
	int ay, by, cy;
	ay = polygons->m[1][i];
	by = polygons->m[1][i + 1];
...
void add_sphere(struct matrix* edges, double cx, double cy, double cz, double r, double step) {
	...
	for (lat = latStart; lat < latStop; lat++) {
		for (longt = longStart; longt < longStop; longt++) {
			...
			if (longt < longStop - 1)
				add_polygon(edges, points->m[0][p0], points->m[1][p0], points->m[2][p0], points->m[0][p1],
				            points->m[1][p1], points->m[2][p1], points->m[0][p2], points->m[1][p2], points->m[2][p2]);
			if (longt > 0)
				add_polygon(edges, points->m[0][p0], points->m[1][p0], points->m[2][p0], points->m[0][p2],
				            points->m[1][p2], points->m[2][p2], points->m[0][p3], points->m[1][p3], points->m[2][p3]);
...
void add_torus(struct matrix* edges, double cx, double cy, double cz, double r1, double r2, double step) {
	...
	for (lat = latStart; lat < latStop; lat++) {
		for (longt = longStart; longt < longStop; longt++) {
			...
			add_polygon(edges, points->m[0][p0], points->m[1][p0], points->m[2][p0], points->m[0][p2], points->m[1][p2],
			            points->m[2][p2], points->m[0][p3], points->m[1][p3], points->m[2][p3]);
			add_polygon(edges, points->m[0][p0], points->m[1][p0], points->m[2][p0], points->m[0][p3], points->m[1][p3],
			            points->m[2][p3], points->m[0][p1], points->m[1][p1], points->m[2][p1]);
		}
	}
	free_matrix(points);
}
...
void draw_lines(struct matrix* points, screen s, zbuffer zb, color c) {
	...
	for (point = 0; point < points->lastcol - 1; point += 2)
		draw_line(points->m[0][point], points->m[1][point], points->m[2][point], points->m[0][point + 1],
		          points->m[1][point + 1], points->m[2][point + 1], s, zb, c);
} // end draw_lines
