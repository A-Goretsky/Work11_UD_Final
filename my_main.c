	...
	for (i = 0; i < lastop; i++) {
		if (op[i].opcode == VARY) {
			...
			for (k = 0; k < num_frames; k++) {
				...
				while (curr) {
					if (strncmp(curr->name, op[i].op.vary.p->name, sizeof(curr->name)) == 0) {
						found = 1;
						break;
					} // end found
					curr = curr->next;
...
/*light* lighting_pass() {
    ...
    int numlights = 0;
    light* lightobj;
    for(ctr = 0; ctr < lightobj; i++) {
        if (op[ctr].opcode == LIGHT) {
            lightobj = (light *)calloc(numlights, sizeof(light));
            lightobj[numlights].x = op[i].op.light.p->s.l->l[0];
            lightobj[numlights].y = op[i].op.light.p->s.l->l[1];
            lightobj[numlights].z = op[i].op.light.p->s.l->l[2];
            lightobj[numlights].r = op[i].op.light.c[0];
            lightobj[numlights].g = op[i].op.light.c[1];
            lightobj[numlights].b = op[i].op.light.c[2];
            numlights++;
        }
    }
    return lightobj;
		...
		for (i = 0; i < lastop; i++) {
			...
			switch (op[i].opcode) {
				...
				case TORUS:
					...
					add_torus(tmp, op[i].op.torus.d[0], op[i].op.torus.d[1], op[i].op.torus.d[2], op[i].op.torus.r0,
				...
				case BOX:
					...
					add_box(tmp, op[i].op.box.d0[0], op[i].op.box.d0[1], op[i].op.box.d0[2], op[i].op.box.d1[0],
